...
<body>
				...
				<div class="row">
								<div class="col">
												<table>
																...
																<tbody>
																				...
																				<tr>
																								<td>Teléfono</td>
																								<td>{{ $lot->lead_phone ?? '—' }}</td>
																				</tr>
																				<tr>
																								<td>Correo</td>
																								<td>{{ $lot->lead_email ?? '—' }}</td>
																				</tr>
																				<tr>
																								<td>Ciudad</td>
																								<td>{{ $lot->city ?? '—' }}</td>
								...
								<div class="col">
												<table>
																<thead class="table-light">
																				<tr>
																								<th colspan="2">Información del Lote</th>
																				</tr>
																</thead>
																<tbody>
																				<tr>
																				...
																				<tr>
																								<td>Área</td>
																								<td>{{ number_format($lot->area, 2) }} m²</td>
																				</tr>
																				<tr>
																								<td>Precio m²</td>
																				...
																				<tr>
																								<td>Precio Total</td>
																								<td>${{ number_format($lot->precioTotal, 2) }}</td>
																				</tr>

																				@if (isset($lot->desarrollo_name))
																								<tr>
																												<td>Desarrollo</td>
																												<td>{{ $lot->desarrollo_name }}</td>
																								</tr>
																				@endif

																				@if (isset($lot->phase_name))
																								<tr>
																												<td>Fase</td>
																												<td>{{ $lot->phase_name }}</td>
																								</tr>
																				@endif

																				@if (isset($lot->stage_name))
																								<tr>
																												<td>Etapa</td>
				...
				<table>
								<thead>
												<tr>
																<th class="purple-header" colspan="3">Enganche / Financiamiento / Monto Financiado</th>
												</tr>
								</thead>
								<tbody>
												<tr>
				...
				<table>
								<thead>
												<tr>
																<th class="purple-header" colspan="2">Mensualidad / Costo Total</th>
												</tr>
								</thead>
								<tbody>
												<tr>
				...
				<div class="row">
								<div class="col summary-box">
												...
												<div style="padding:8px; text-align:center; font-weight:700;">
																${{ number_format($lot->plusvaliaTotal - $lot->precioTotal, 2) }}</div>
								</div>
								<div class="col summary-box">
								...
								<tbody>
												@foreach ($lot->years as $r)
																<tr>
																				...
																				<td class="{{ $r['plusvaliaAcum'] > 0 ? 'text-success' : '' }}">
																								+${{ number_format($r['plusvaliaAcum'], 2) }}</td>
																				<td class="{{ $r['roiAnual'] > 0 ? 'text-primary' : '' }}">{{ number_format($r['roiAnual'], 2) }}%
																				</td>
																</tr>
												@endforeach
								</tbody>
