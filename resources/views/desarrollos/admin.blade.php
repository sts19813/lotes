                    ...
                    <tbody>
                        @foreach ($lots as $lot)
                            <tr>
                                ...
                                <td>
                                    @if ($lot->png_image || $lot->svg_image)
                                        <div class="image-container">
                                            @if ($lot->png_image)
                                                <img data-src="{{ asset($lot->png_image) }}" class="lazy-img base-img" alt="PNG"
                                                    loading="lazy">
                                            @endif
                                        </div>
                                    @endif
                                ...
                                <td>
                                    <a href="{{ route('desarrollos.configurator', $lot->id) }}" class="btn btn-sm btn-primary">
                                        {{ __('messages.buttons.configure') }}
                                    </a>
                                    <a href="{{ url('iframe/' . $lot->id) }}" class="btn btn-sm btn-secondary"
                                        style="background-color: #3FB549 !important" target="_blank">
                                        {{ __('messages.buttons.iframe') }}

                                    </a>
                                    <a href="{{ route('desarrollos.edit', $lot->id) }}" class="btn btn-sm btn-warning">
